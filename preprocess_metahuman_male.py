# Convert sanitized male OBJ meshes to compressed coordinate/face arrays.
#
...
def process_sanitized_male_obj(obj_path, output_dir):
    current_object = None
    object_vertices = {}
    global_vertices = []
    global_faces = []
    ...
    if not object_vertices:
        ...
        # --- MIRROR FEMALE PROTOCOL: Extract the single maximum continuous joined block ---
...
def run_male_extraction():
    ...
    print("Starting topology-invariant geometry extraction for Male Cohort...")
    
    obj_files = [f for f in os.listdir(SOURCE_DIR) if f.endswith('.obj')]
    for file in obj_files:
        v_shape = process_sanitized_male_obj(os.path.join(SOURCE_DIR, file), EXPORT_DIR)
