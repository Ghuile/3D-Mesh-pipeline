# Evaluate the male cohort under Gaussian coordinate perturbations.
#
...
# -----------------------------------------------------------------------------
def run_male_robustness_pipeline():
    print("==========================================================================")
    print("INITIALIZING NOISE ROBUSTNESS STRESS-TEST ENGINE: MALE COHORT (AOI)")
    print("==========================================================================\n")
    ...
    output_plot_path = os.path.join(parsed_dir, "male_noise_degradation_profile.png")
    
    # Set fixed architectural attributes for male branch (Aoi)
    num_vertices = 66991
    input_size = num_vertices * 3  # 200,973 input channels
    weight_file = 'mmetahuman_autoencoder_trained.pth'
    male_folder = 'male_medium_average'
    ...
    # Filter registry exclusively for the male cohort
    male_df = df_registry[df_registry['cohort'] == 'male']
    print(f" -> Detected {len(male_df)} valid entries for the Male Cohort.")
    ...
    for _, row in male_df.iterrows():
        ...
        full_mesh_path = os.path.join(parsed_dir, male_folder, filename)
    ...
    # -----------------------------------------------------------------------------
    # Noise thresholds: 0mm, 2mm, 5mm, 10mm, and 20mm spatial sweeps
    ...
    # Subplot 1: R² Performance Trends (Uses blue theme layout for male representation)
    ax1.plot(noise_levels_mm, gpr_r2_scores, marker='o', color='#1f77b4', linewidth=2.5, label='GPR Bayesian Head')
    ...
    ax2.plot(noise_levels_mm, gpr_mae_scores, marker='o', color='#1f77b4', linewidth=2.5, label='GPR Bayesian Head')
    ...
    plt.suptitle("MALE COHORT BRANCH (AOI): Downstream Model Performance Degradation Under Noise Stress", 
